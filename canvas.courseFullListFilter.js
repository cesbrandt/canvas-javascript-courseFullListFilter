 ...
 * @return undefined
 */
function callAPI(context, page, getVars, oncomplete, oncompleteInput, lastPage, firstCall) {
	...
	if(page === 1 || audit) {
		$.when(callAJAX(callURL, expandedVars[0])).then(function(json, status, xhr) {
			if(!audit) {
				if(xhr.getResponseHeader('Link') !== null) {
					var lastPage = parseInt(xhr.getResponseHeader('Link').match(/\bpage=(\d+\b)(?=[^>]*>; rel="last")/)[1]);
					if(page < lastPage) {
						callAPI(context, (page + 1), getVars, oncomplete, oncompleteInput, lastPage, json);
					} else {
				...
				if(xhr.status != 200) {
					...
					var results = (firstCall.length == 1 && isEmpty(firstCall[0])) ? json.events : $.merge(firstCall, json.events);
					if(json.events.length === 100) {
						page = xhr.getResponseHeader('link').match(/\bpage=[^&]*(?=[^>]*>; rel="next")/)[0].split('=')[1];
						callAPI(context, page, getVars, oncomplete, oncompleteInput, null, results);
					} else {
		...
		for(var i = (page + 1), j = (callsToMake.length - 1); i <= lastPage; i++) {
...
function replaceList(page) {
	if($('#filterCourseListSubmitContainer').children().length === 0) {
		$('#filterCourseListSubmitContainer').append($('<a />').addClass('btn btn-primary').html('<i class="icon-search"></i> Filter').click(function(e) {
			e.preventDefault();
			replaceList(0);
		}));
	}
	...
	var table = $('<table />').addClass('ic-Table ic-Table--hover-row').html('<thead><tr></tr><tr><th>ID</th><th>Course Code</th><th>Name</th><th>SIS ID</th><th>State</th><th>Links</th></tr></thead><tbody></tbody><tfoot></tfoot>');
	$.each(window.sortedList[page], function() {
		table.find('tbody').append(this.courseList);
	});
	$('.courseList table').replaceWith(table);

	$('.courseList table thead tr:first').replaceWith(generatePagination(page, 'Head'));
	$('.courseList table tfoot').html('').append(generatePagination(page, 'Foot'));
}
...
function generatePagination(page, slot) {
	var pagination = $('<tr />').html('<th colspan="6"><div class="ui-buttonset"></div></th>');
	var paginationTitle = 'filterCourseListPagination';
	for(var i = 0; i < window.sortedList.length; i++) {
		var pageTab = paginationTitle + i + slot;
		var active = i === page;
		pagination.find('th div').append($('<input />').addClass('ui-helper-hidden-accessible').attr({type: 'radio', id: pageTab, name: paginationTitle + slot}).change(function(e) {
			replaceList(parseInt($(this).attr('id').replace(/[^\d]/g, '')));
		}));
		if(active) {
			pagination.find('#' + pageTab).attr({checked: 'checked'});
		}
		pagination.find('th div').append($('<label />').addClass((active ? 'ui-state-active ' : '') + 'ui-button ui-widget ui-state-default ui-button-text-only' + (i === 0 ? ' ui-corner-left' : (i === window.sortedList.length - 1 ? ' ui-corner-right' : ''))).attr({for: pageTab, role: 'button', ariaDisabled: 'false'}).html('<span class="ui-button-text">' + (i + 1) + '</span>'));
	}
	return pagination;
}
...
function sortList() {
	...
	if($('#filterTermID option:selected').val() !== '') {
		filtered = $.grep(filtered, function(course) {
			return course.enrollment_term_id === parseInt($('#filterTermID option:selected').val());
		});
	}
	...
	var sorted = filtered.sort(function(course1, course2) {
		...
		if(isNaN(course1Numeric) && isNaN(course2Numeric)) {
			...
			if(course1Alpha === course2Alpha) {
				var course1Numeric2 = parseInt(course1SortBy.replace(regexNumeric, ''), 10);
				var course2Numeric2 = parseInt(course2SortBy.replace(regexNumeric, ''), 10);
				return course1Numeric2 === course2Numeric2 ? 0 : course1Numeric2 > course2Numeric2 ? 1 : -1;
			} else {
	...
	window.sortedList = [];
	var displayLength = parseInt($('#filterDisplayNumber option:selected').val());
	if(displayLength === 0) {
...
(function() {
	if($('ul.courses').length > 0) {
		...
		var filter = $('<div />').addClass('ic-Action-header header-bar row-fluid pad-box-mini border border-trbl border-round-t').css({marginBottom: 0, paddingBottom: 0}).html('<div id="filterCourseListSelectorsContainer" class="span10"><input type="text" id="filterByName" placeholder="Course Name">&nbsp;' + $('#enrollment_term_id').clone().attr({id: 'filterTermID'}).wrap('<div />').parent().html() + '&nbsp;<select id="filterSortBy"><option value="1">Course Code (A-Z)</option><option value="2">Course Code (Z-A)</option><option value="3">Name (A-Z)</option><option value="4">Name (Z-A)</option><option value="5">Oldest - Newest</option><option value="6" selected="selected">Newest - Oldest</option></select>&nbsp;<select id="filterDisplayNumber"><option value="50" selected="selected">Display x Shells</option><optgroup label="-----"><option value="10">10</option><option value="25">25</option><option value="50">50 (Default)</option><option value="100">100</option></optgroup><optgroup label="-----"><option value="0">All</option></optgroup></select></div><div id="filterCourseListSubmitContainer" class="span2 align-right"></div>');
		...
		$('ul.courses').replaceWith(contentBox.append(filter, $('<div />').addClass('pad-box-mini border border-rbl border-round-b courseList').html('<table style="width: 100%;"><thead><tr><th>Please be patient while the course list loads.<br />This may take a few minutes depending on how many courses fall under this account.</th></tr></thead><tbody><tr><td style="text-align: center;"><i id="audio_record_holder" style="display: inline-block; background-position: 0; box-shadow: none; width: 31px; height: 31px; margin: 0;"></i></td></tr></tbody></table>')));
	}
	getCourses();
})();
